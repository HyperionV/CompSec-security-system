...
class SecurityAppGUI:
    ...
    def handle_login_success(self, user_info):
        ...
        # Create and show MFA screen
        if not self.screens['mfa']:
            ...
            self.screens['mfa'].reset_for_new_user(user_info)
    ...
    def handle_mfa_success(self):
        ...
        # Create main application screen if not exists
        if not self.screens['main_app']:
            ...
            # Refresh the main app screen for the new user
            self.screens['main_app'].refresh_all_tabs()
    ...
    def handle_logout(self):
        ...
        # Log the logout
        if self.user_session.user_info:
            self.managers['logger'].log_activity(
                user_id=self.user_session.user_info['id'],
                action='logout',
                status='success',
                details='User logged out via GUI'
            )
